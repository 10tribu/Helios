...
| `power-unit` | `W` \| `kW` \| `adaptive` | `kW` | Unit for every power readout (chips, tooltips). Energy totals follow it by default (`kW` pairs with `kWh`, `W` with `Wh`), unless `energy-unit` below sets one of its own. `adaptive` is a rule rather than a unit: each value is printed in watts below a kilowatt and in kilowatts above, so a device that swings between 2 kW and 80 W reads right at both ends. |
| `energy-unit` | `auto` \| `Wh` \| `kWh` \| `adaptive` | `auto` | Unit for every energy total (the day curve, the detail panel, the timeline's day totals). `auto` follows `power-unit`; set `Wh` or `kWh` to pick one independently of the power unit above, e.g. precise `W` chips alongside `kWh` totals. `adaptive` switches at one kilowatt-hour, and an `adaptive` power unit carries into `auto`. |
